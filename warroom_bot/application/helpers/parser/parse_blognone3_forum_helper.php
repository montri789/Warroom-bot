<?php
	function parse_blognone3_forum($fetch,$page,$debug=false)
	{
		$log_unit_name = 'parse_blognone3_forum';
		...
		else
		{
			if($parsed_posts_count == 0 && $page->sub_comment == 0) 
			{
				...
				$mm = $str2[1];
				$dd = $pday[0];
				$post_date = thYear_decoder($yy)."-".$mm."-".$dd." ".$pyear[1];
			...
			foreach($comments as $k=>$c)
			{
				...
				if($i < $parsed_posts_count-1)
				{
					...
					$comment_date = thYear_decoder($date[2])."-".$date[1]."-".$date[0]." ".$str[1];
